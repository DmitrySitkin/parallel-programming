...
int* parallel_dijkstra(int* graph, int start, int count_vertex) {
	...
	int flag = 1;
	int flag_finish_find_current_min_distation_in_sigment_of_rank = 1;
	...
	MPI_Status st;
	if (rank == 0) {
		MPI_Scatterv(graph, count_element_graph, dist_graph, MPI_INT, part_graph, count_element_graph[rank], MPI_INT, 0, MPI_COMM_WORLD); //send graph
		...
		while (!queue.empty()) {
			...
			MPI_Bcast(&current_vertex, 1, MPI_2INT, 0, MPI_COMM_WORLD); //send vertex and dist
